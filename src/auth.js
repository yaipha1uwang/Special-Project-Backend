 ...
 */
import CreateAuth from "./frontend_create/@auth/create.js"
  ...
  providers: [
    Credentials({
      ...
      authorize: async (credentials) => {
        ...
        // 1. Fetch user to verify account exists
        const user = await adapter.getUserByEmail(email);
        if (!user) return null;

        // 2. Fetch active verification token matching email and OTP code
        const sql = `
        ...
        // 3. Verify hasn't expired
        if (new Date(dbToken.expires) < new Date()) {
          // Consume (delete) the expired token immediately so it can't sit around
          await adapter.useVerificationToken({ identifier: email, token: otp });
          return null; // Expired 
        }

        // 4. Consume the valid token (deletes it from DB to prevent replay attacks)
        await adapter.useVerificationToken({ identifier: email, token: otp });
